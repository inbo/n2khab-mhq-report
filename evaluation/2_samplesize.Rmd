...
We will first assume that the habitat condition is binary variable (good or not-good) and analyse which difference between proportions of habitat in not-good condition we can detect.
...
To determine the detectable difference in habitat proportions in not-good condition between two points in time, we apply the *McNemar test* [@chow2003] using the function `McNemar.Test` in the R package TrialSize [@zhang2020].
...
data.frame(
  t_1 = c("not-good", "good"),
  not_good = c("no change", "good -> not-good"),
  good = c("not-good -> good", "no change")
) %>%
  select("$t_1$" = t_1, "not-good" = not_good, good) %>%
  kable(booktabs = TRUE,
        escape = FALSE, 
        format = format_table) %>%
  kable_styling() %>%
...
Suppose that the net proportion with not-good quality decreases with 10 %, then the dynamics are equal to 2 when 15% of the habitat area changes from good to not-good and 5% of the habitat area from not-good to good. The higher the dynamics, the higher the minimal detectable difference.
...
Continuous, discrete or categorical variables that measure habitat quality are more sensitive to changes than a binary variable (good or not-good habitat condition). Examples of such variables are:
...
verschil_aandeel_gunstig_detecteerbaar %>%
  ...
  kable(caption = "Minimum detectable difference in proportion of favourable indicators that can be expected for different sample sizes with a power $pi$ of 0.80",
        booktabs = TRUE, 
        format = format_table) %>%
  kable_styling() %>%
  collapse_rows(1)
