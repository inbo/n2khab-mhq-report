...
calc_power_simr <- function(model, sim_n, sim_effect) {
  ...
  sim <- powerSim(model_sim, nsim = 100)
...
To verify the results we calculated the power for habitat type 9120 using the `simr` package. The power is close to 0.8 for all combinations of sample size and effect size in Table \@ref(tab:tabdiffdetect1). So both methods give similar results. The estimated power using `simr` is shown in Table \@ref(tab:) in Appendix \@ref(annexsimr).     
...
model_ks_9120_poisson_s <- summary(model_ks_9120_poisson)

plotQQunif(model_ks_9120_poisson)

E1 <- resid(model_ks_9120_poisson, type = "pearson")
N <- nrow(n_keyspecies_9120)
p <- length(fixef(model_ks_9120_poisson)) + 1
...
```{r}
simr_select_results <- simr_result2 %>%
  filter(nspecies_p1 == 3) %>%
  filter(power >= 0.75) %>%
  group_by(n) %>%
  slice_min(lcl, n = 1) %>%
  ungroup()
...
verschil_keyspecies_simr <- calc_power_simr_df(model_encroachement_9120, verschil_encroachment_detecteerbaar_9120)
...
check <- calc_power_simr(model_ks_9120_poisson, sim_n = 40, sim_effect = -0.1)
