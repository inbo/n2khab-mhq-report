...
voorwaarden_mean_p1 <- voorwaarden_bos_select %>%
  group_by(voorwaarde_engels) %>%
...
verschil_voorwaarden_91xx <- verschil_voorwaarden %>%
  filter(type == "91xx") %>%
  tidyr::expand(nesting(voorwaarde_engels, sd_diff), n = c(40, 80, 170))
  
verschil_voorwaarden_detecteerbaar_91xx <- sim_data %>%
  filter(n %in% c(40, 80, 170)) %>%
  mutate(d = round(d, 2)) %>%
  filter(alpha == 0.05) %>%
  distinct(n, d) %>%
  left_join(verschil_voorwaarden_91xx, by = "n") %>%
  left_join(voorwaarden_mean_p1, by = "voorwaarde_engels") %>%
...
```{r}
n_keyspecies_91xx <- voorwaarden_bos_select %>%
 # filter(type == "9120") %>%
...
model_ks_91xx_poisson <- glmer(formula = n ~ periode + (1|id_group),
              data = n_keyspecies_91xx,
              family = "poisson")

model_ks_91xx_poisson_summary <- summary(model_ks_9120_poisson)
...
verschil_keyspecies_detecteerbaar_91xx <- verschil_voorwaarden_detecteerbaar_91xx %>%
...
verschil_keyspecies_simr <- calc_power_simr_df(model_ks_9120_poisson, verschil_keyspecies_detecteerbaar_91xx)
...
```{r}
encroachement_91xx <- voorwaarden_bos_select %>%
 # filter(type == "9120") %>%
...
encroachement_91xx %>%
  ggplot(aes(x = cover, fill = periode)) +
  geom_density()


model_encroachement_91xx <- lmer(formula = cover ~ periode + (1|id_group),
              data = encroachement_91xx)

model_encroachement_91xx_summary <- summary(model_encroachement_91xx)

plotQQunif(model_encroachement_91xx)
