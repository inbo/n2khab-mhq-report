...
+ We propose following changes in the monitoring design of the open terrestrial habitat types and the stream habitat type (3260):
  + Decrease the monitoring cycle from 12 years to 6 years making it possible to assess the habitat quality trend after 12 years of monitoring (= 2 monitoring cycles of 6 years) instead of 24 years (= 2 monitoring cycles of 12 years);
  + Decrease the oversampling within Special Areas of Conservation (SAC) from 170 to 80 sampling units per habitat type, as a sample size of 80 is sufficient to differentiate between favourable and unfavourable habitat quality for most habitat types; 
  + Decrease the oversampling for habitat subtypes from 80 to 40 sampling units per subtype, as information on the conservation status for habitat subtypes is of lower priority and is not required for the European reporting (Habitat Directive).
...
+ The changes in the monitoring design are summarized in Table \@ref(tab:tababstract).
...
table_abstract <- tibble(hab_cat = c("terrestrial open habitat", "terrestrial open habitat", "stream habitat", "stream habitat", "standing water habitat", "standing water habitat"),
                         ...
                         sampling_unit = c("36 m diameter plot", "36 m diameter plot", "100 m river segment", "100 m river segment", "entire water surface", "entire water surface"))
...
+ Table \@ref(tab:tababstract2) summarizes the minimal detectable difference (mmd) for habitat quality status and trend. 
...
+ Het meetnet voor biotische habitatkwaliteit (Monitoring scheme for biotic Habitat Quality of MHQ) heeft als doel om per habitattype en -subtype te bepalen welk aandeel van het habitat zich in een slechte staat bevindt. De habitatkwaliteit is gunstig wanneer dit aandeel kleiner is dan 25%. Deze informatie is nodig om het criterium 'structuur en functies' te beoordelen en de globale staat van instandhouding te bepalen per habitattype, en is daarom van groot belang voor de 6-jaarlijkse rapportage in kader van de Habitatrichtlijn.    
...
+ We stellen volgende wijzigingen voor aan het meetnetontwerp voor de open terrestrische habitattypen en het aquatisch habitattype 3260:
  + vermindering van de meetcyclus van 12 jaar naar 6 jaar, zodat het mogelijk wordt om na 12 jaar (2 meetcycli van 6 jaar) de trend in habitatkwaliteit te bepalen i.p.v. na 24 jaar (2 meetcycli van 12 jaar);
  + vermindering van de overbemonstering binnen de Speciale Beschermingszones (SBZ) van 170 naar 80 steekproefeenheden per habitattype omdat een steekproefgroote van 80 voor de meeste habitattypen volstaat om een onderscheid te maken tussen een gunstige en ongunstige kwaliteit.
  + vermingering van de overbemonstering binnen de habitatsubtypen van 80 naar 40 steekproefeenheden per subtype gezien de informatie over de staat van instandhouding van habitatsubtypen een lagere prioriteit heeft en niet noodzakelijk is voor de EU rapportage.

+ In het originele meetnetontwerp voor habitattypen in stilstaande wateren werden alle plassen groter dan 5 ha geselecteerd en werd er een steekproef getrokken uit de set van plassen kleiner dan 5 ha. We stellen voor om ook een steekproef te nemen uit de set van plassen met een oppervlakte tussen 5 ha en 50 ha om de haalbaarheid en kostenefficiëntie te verhogen. We stellen ook voor om de overbemonstering in SBZ te laten vallen omdat de meeste plassen in SBZ gelegen zijn en de steekproef dus reeds voldoende plassen binnen SBZ bevat.
...
+ De veranderingen aan het meetnetontwerp vatten we samen in Tabel \@ref(tab:tababstractd).
...
table_abstract <- tibble(hab_cat = c("terrestrisch open", "terrestrisch open", "rivier", "rivier", "stistaande wateren", "stistaande wateren"),
                         ...
                         n_subtype = c(80, 40, NA, NA, 40, 40),
                         n_year = c("247", "258", "15", "17", "43.5", "42"),
                         sampling_unit = c("36 m diameter plot", "36 m diameter plot", "100 m rivier segment", "100 m rivier segment", "volledige plas", "volledige plas"))
