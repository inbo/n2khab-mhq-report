...
-   the fraction of the habitat area in good condition (as assessed following the criteria and indicators by @oosterlynck2020) and its trend
...
Figure \@ref(fig:figstatustrend) shows the different conclusions that can be drawn based on the estimates (and the confidence interval) for habitat quality status and trend. Note that the habitat quality status is unknown when the confidence interval includes the 25% threshold. Also note that we need to define an upper and a lower threshold for the habitat quality trend to differentiate between a stable and an unknown trend. The confidence interval includes 0% change when the trend is stable or unknown. However when the confidence interval is very wide we have no idea whether there is an increase or a decrease in habitat quality and the therefore the trend is unknown.    
  
  
```{r figstatustrend, fig.cap= "Possible coclusions for habitat quality status and habitat quality trend"}
