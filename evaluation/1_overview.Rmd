...
plot_status <- tibble(prop_slecht = c(0.15, 0.27,0.55),
  ...
  labs(x = "Status category", y = "Habitat proportion in not-good condition", colour = "Conclusion") +
...
plot_trend <- tibble(trend = c(-0.45, 0.05, -0.09, 0.33),
                     ci = c(0.25, 0.1, 0.35, 0.2),
  ...
  ggplot(aes(x = trend_class, y = trend, ymin = trend_lcl, ymax = trend_ucl, colour = trend_class)) +
    geom_point(alpha = 0.8, size = 2) +
    geom_errorbar(width = 0.2) +
    geom_hline(yintercept = c(-0.25, 0, 0.25), linetype = 2) +
    labs(x = "Trend category", y = "Change in habitat proportion in not-good condition", colour = "Conclusion") +
    scale_y_continuous( breaks = c( -0.25, 0, 0.25), labels = c("Lower threshold \n(to be defined)", "Reference \n(0 % change)", "Upper threshold \n(to be defined)")) +
