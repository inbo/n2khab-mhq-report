...
The total sample size almost doubles and the available resources for field work have decreased. This makes that the sample size that results from the current sampling frame is not feasible. Therefore we will explore two alternative scenarios.
...
samplesize_alt_overview %>%
  ...
  kable(caption = "Alternative sample size scenarios compared to the original design: number of sampling units per type and total number of water surfaces per type",
        booktabs = TRUE, 
        format = format_table) %>%
  kable_styling() %>%
  column_spec(c(4,5,6,7), width = "1.8cm") %>%
