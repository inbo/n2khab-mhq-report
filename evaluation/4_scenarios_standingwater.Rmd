...
The sampling unit for the standing water monitoring scheme is the entire water body. The original design for the standing water habitat types in @westra2014 consists of a stratified sample with a combination of following strata:
...
The total sample size almost doubles. Since the available resources for field work has not increased, the sample size that results from the current sample frame is not feasible. Therefore we will explore two alternative scenario's.

## Alternative scenarios

We propose the following alternative scenario's:
...
samplesize_alt_total %>%
  ...
  kable(caption = "Alternative sample size scenarios compared the to original design: total number of sampling units and total number of watersurfaces",
        booktabs = TRUE,
        escape = FALSE) %>%
  kable_styling() %>%
...
samplesize_alt_overview %>%
  ...
  kable(caption = "Alternative sample size scenarios compared the to original design: number of sampling units per type and total number of watersurfaces per type",
        booktabs = TRUE) %>%
  kable_styling() %>%
  column_spec(c(4,5,6,7), width = "1.8cm") %>%
...
In Table \@ref(tab:tab-wsyear) we compare the number of water bodies that have to be measured per year in the original design and the updated design. Since measuring large water bodies require more effort, we show the numbers per area class. We see that in the updated design a slightly higher number of large water bodies have to be measured.
