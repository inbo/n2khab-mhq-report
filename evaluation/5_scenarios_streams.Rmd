# Sample design for stream habitat types
...
The sample frame of original design is based on version 1.4 of the 3260 habitat map [@leyssen2012]. A sampling unit corresponds with a river segment of 100 meter. 
...
sample_size_strata %>%
  ...
  select(sac, "catchmentarea" = catchment_area, "length 3260 (meters)" = length_3260, n = n_finite_stratum) %>%
  kable(booktabs = TRUE) %>%
...
In the updated design we use version 1.7 of the habitat map of 3260 [@inbo2020_3] as the sample frame. It can be imported in R by using the function `read_habitatstreams()` of the n2khab package [@vanderhaeghe2022]. Another change in the sample design is that we will no longer make use of the stratum 'catchement area' as explained above. Finally, we will use the same design parameters as the terrestrial habitat types in order to get better information on trends in habitat quality:
...
In the table below the sample size for both versions are compared. The updated design results in a slightly higher sample size per year.
